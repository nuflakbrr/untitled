...
func (r *CategoryRepository) FindByID(ctx context.Context, id string, scopeTenantID *string) (*domain.Category, error) {
	query := `
		SELECT id, tenant_id, name, slug, description, created_at, updated_at, deleted_at,
		       (SELECT COUNT(*) FROM events e WHERE e.category_id = event_categories.id AND e.deleted_at IS NULL)
	...
	err := r.db.QueryRow(ctx, query, args...).Scan(
		...
		&category.Description, &category.CreatedAt, &category.UpdatedAt, &category.DeletedAt, &category.EventsCount,
	)
...
func (r *CategoryRepository) FindAccessible(ctx context.Context, id, tenantID string) (*domain.Category, error) {
	...
	`, id, tenantID).Scan(
		&category.ID, &category.TenantID, &category.Name, &category.Slug,
		&category.Description, &category.CreatedAt, &category.UpdatedAt, &category.DeletedAt,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrCategoryNotFound
	}
...
func (r *CategoryRepository) Delete(ctx context.Context, id string, scopeTenantID *string) error {
	query := "UPDATE event_categories SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL"
	args := []any{id}
	if scopeTenantID != nil {
		query += " AND tenant_id = $2"
		args = append(args, *scopeTenantID)
...
func (r *CategoryRepository) PermanentDelete(ctx context.Context, id string, scopeTenantID *string) error {
	query := "DELETE FROM event_categories WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		query += " AND tenant_id = $2"
		args = append(args, *scopeTenantID)
	...
	tag, err := r.db.Exec(ctx, query, args...)
	if err != nil {
		return fmt.Errorf("permanently delete event category: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrCategoryNotFound
	}
...
func (r *CategoryRepository) Restore(ctx context.Context, id string, scopeTenantID *string) error {
	query := "UPDATE event_categories SET deleted_at = NULL, updated_at = NOW() WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		query += " AND tenant_id = $2"
		args = append(args, *scopeTenantID)
	...
	tag, err := r.db.Exec(ctx, query, args...)
	if err != nil {
		return fmt.Errorf("restore event category: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrCategoryNotFound
	}
