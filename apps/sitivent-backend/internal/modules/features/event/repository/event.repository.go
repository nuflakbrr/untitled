...
func (r *EventRepository) Restore(ctx context.Context, id string, scopeTenantID *string) error {
	query := "UPDATE events SET deleted_at = NULL, updated_at = NOW() WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		query += " AND tenant_id = $2"
		args = append(args, *scopeTenantID)
