...
function TiptapImage(props: NodeViewProps) {
  ...
  const resize = useCallback(
    ...
    [resizing, resizeInitialMouseX, resizingPosition, resizeInitialWidth, updateAttributes]
  );
  ...
  useEffect(() => () => {
      ...
      // We need to check if the node is actually being deleted from the document
      // and not just being unmounted/re-rendered.
      // Tiptap's deleteNode handles this, but for backspace we check the document state.
      setTimeout(() => {
