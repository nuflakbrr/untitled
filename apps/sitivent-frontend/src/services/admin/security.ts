...
export async function verifyPermission(permissionName: string) {
  ...
  return Boolean(
    session?.permissions.includes(permissionName) ||
    session?.roles.some((role: string) => role.toLowerCase().includes('admin'))
  );
}
...
export async function createAuditLog(params: {
  ...
  try {
    return (
      await api.post('/core/v1/audit-logs', {
        reff_type: params.table,
        reff_id: params.recordId,
