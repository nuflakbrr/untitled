...
const normalizeUser = (user: Record<string, unknown>): User => {
  ...
  return {
    ...user,
    deletedAt: (user.deletedAt ?? user.deleted_at) as string | null | undefined,
    roleId,
    roles:
...
export async function getUsers(
  ...
  try {
    ...
    return {
      success: true,
      data: (body.data ?? []).map(normalizeUser),
...
export async function getUserById(id: string): Promise<UserResponse> {
  try {
    return { success: true, data: normalizeUser((await api.get(`${endpoint}/${id}`)).data.data) };
  } catch {
...
export async function createUser(values: UserValues): Promise<UserResponse> {
  try {
    const result = await api.post(endpoint, { ...values, full_name: values.name });
    revalidatePath('/admin/managements/users');
    return { success: true, data: result.data.data, message: 'User berhasil dibuat.' };
  } catch {
...
export async function updateUser(id: string, values: UserValues): Promise<UserResponse> {
  try {
    const result = await api.put(`${endpoint}/${id}`, { ...values, full_name: values.name });
    revalidatePath('/admin/managements/users');
    return { success: true, data: result.data.data, message: 'User berhasil diperbarui.' };
  } catch {
...
export async function permanentlyDeleteUser(id: string): Promise<UserResponse> {
  try {
    const result = await api.delete(`${endpoint}/${id}/permanent`);
    revalidatePath('/admin/managements/users');
...
export async function banUser(id: string): Promise<UserResponse> {
  try {
    await api.post(`${endpoint}/${id}/ban`, { reason: 'Diblokir oleh administrator.' });
    revalidatePath('/admin/managements/users');
    return { success: true, message: 'User berhasil diban.' };
  } catch {
    return { success: false, error: 'Gagal memban user.' };
  }
}
export async function unbanUser(id: string): Promise<UserResponse> {
  try {
    await api.post(`${endpoint}/${id}/unban`);
    revalidatePath('/admin/managements/users');
    return { success: true, message: 'User berhasil di-unban.' };
  } catch {
    return { success: false, error: 'Gagal melakukan unban user.' };
  }
}
export async function getCurrentUserData() {
  const session = await auth.api.getSession();
  return { permissions: session?.permissions ?? [], roles: session?.roles ?? [] };
}
