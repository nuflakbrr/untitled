...
export async function getRegistrations(
  ...
  eventId?: string,
  status?: string
): Promise<RegistrationPaginationResponse> {
  try {
    const body = (
      await api.get(endpoint, { params: { page, limit, search, event_id: eventId, status } })
    ).data;
    const data = (body.data ?? []).map(normalizeRegistration);
...
export async function getEventsForFilter() {
  try {
    const items =
      (await api.get('/features/v1/events', { params: { limit: 100 } })).data.data ?? [];
    return items.map((item: Record<string, unknown>) => ({ id: item.id, title: item.title }));
  } catch {
    return [];
  }
}
export async function exportRegistrationsData(
  ...
  const result = await api.get(`${endpoint}/event/${eventId || 'all'}/export`, {
...
export async function cancelRegistration(registrationId: string): Promise<any> {
  try {
    await api.delete(`${endpoint}/${registrationId}`);
