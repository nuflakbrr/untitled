...
func Setup(router *gin.Engine, db *pgxpool.Pool, cfg *config.Config) {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:5173", "http://localhost:3000", "http://localhost:3001", "http://localhost:8002", "http://localhost:8081", "https://app.untitled.id", "https://jesuit.venturo.pro", "https://skeleton.venturo.id"},
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"status":  "ok",
			"message": "Untitled API is running",
		})
	})
