...
func (r *CategoryRepository) FindAll(ctx context.Context, tenantID *string) ([]*domain.Category, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&category.ID, &category.TenantID, &category.Name, &category.Slug, &category.Description, &category.CreatedAt, &category.UpdatedAt, &category.DeletedAt); err != nil {
