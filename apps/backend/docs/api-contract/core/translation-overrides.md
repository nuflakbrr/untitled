...
- Setiap client punya **slug** DNS-safe unik (contoh `acme`, `untitled-demo`) yang dipakai FE untuk bootstrap tanpa login
...
- **Slug discovery** — FE harus tahu slug client-nya sebelum bootstrap. Source umum: subdomain (`acme.untitled.id` → `acme`), build-time env var, atau user preference di localStorage.
