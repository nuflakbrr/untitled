...
export default async function GalleriesPage() {
  await requireSession('galleries.read');
      ...
      >
        <Box>
          ...
          <Typography color="text.secondary" sx={{ mt: 0.75 }}>
            Kelola gambar dan dokumentasi event.
          </Typography>
        </Box>
        <Button component="a" href="/dashboard/galleries/create" variant="contained">
          Tambah galeri
        </Button>
      </Box>
      <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 }, borderRadius: 2, overflow: 'hidden' }}>
        {result.data ? (
          <GalleryTable rows={result.data} />
        ) : (
