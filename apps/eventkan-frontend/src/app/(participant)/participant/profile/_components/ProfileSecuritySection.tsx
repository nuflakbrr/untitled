...
export default function ProfileSecuritySection() {
  ...
  return (
    <>
      <section id="security" className="mt-4.5 rounded-[24px] border border-eventkan-ink/10 bg-eventkan-surface p-6.5 shadow-[0_18px_50px_rgba(17,35,63,.08)]">
        <h2 className="font-display text-[22px] font-extrabold tracking-[-.03em] text-eventkan-ink">Keamanan Akun</h2>
        <p className="mt-1 text-[13px] text-eventkan-muted">Ganti password untuk menjaga keamanan akun.</p>
        <form className="mt-3.5" onSubmit={security.form.handleSubmit(security.submitPasswordChange)}>
          <FieldGroup className="gap-3.5">
            ...
            <div className="flex justify-end pt-1">
              <Button type="submit" disabled={security.isChanging} className="rounded-full bg-eventkan-accent px-4.5 font-bold text-white hover:bg-[#e86636]">{security.isChanging ? 'Mengubah...' : 'Ganti Password'}</Button>
            </div>
          </FieldGroup>
        </form>
      </section>
