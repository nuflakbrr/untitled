    ...
    const data = await Promise.all(
      (body.data ?? []).map(
        async (role: { id: string; permissions?: { id: string; name: string }[]; deleted_at?: string | null }) => {
          ...
          const permissionIDs = (await api.get(`${roles}/${role.id}/permissions`)).data.data ?? [];
          return {
            ...role,
            deletedAt: role.deleted_at,
            permissions: permissionIDs
              .map((id: string) =>
                allPermissions.find((permission: { id: string }) => permission.id === id)
              )
              .filter(Boolean),
          };
        }
...
export async function getRoles(page = 1, limit = 10, search = '', includeDeleted = false) {
  return list(page, limit, search, includeDeleted);
}
export async function getRoleByName(name: string): Promise<RoleResponse> {
  try {
    return { success: true, data: (await api.get(`${roles}/${name}`)).data.data };
  } catch {
...
export async function getRoleById(id: string): Promise<RoleResponse> {
  try {
    const role = (await api.get(`${roles}/${id}`)).data.data;
    const [permissionIDs, permissionResponse] = await Promise.all([
      api.get(`${roles}/${id}/permissions`),
      api.get('/core/v1/roles/permissions', { params: { limit: 1000 } }),
    ]);
    const ids = permissionIDs.data.data ?? [];
    const permissions = (permissionResponse.data.data ?? []).filter((permission: { id: string }) =>
...
export async function createRole(values: RoleValues): Promise<RoleResponse> {
  try {
    const result = await api.post(roles, values);
    revalidatePath(path);
    return { success: true, data: result.data.data, message: 'Role berhasil dibuat.' };
  } catch {
...
export async function updateRole(name: string, values: RoleValues): Promise<RoleResponse> {
  return updateRoleById(name, values);
}
export async function updateRoleById(id: string, values: RoleValues): Promise<RoleResponse> {
  try {
    const result = await api.put(`${roles}/${id}`, values);
    revalidatePath(path);
    return { success: true, data: result.data.data, message: 'Role berhasil diperbarui.' };
  } catch {
