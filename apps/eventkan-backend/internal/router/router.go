...
import (
	...
	// Core modules for EVENTKAN Multi-Tenancy
	"eventkan-backend/internal/modules/core/auth"
	"eventkan-backend/internal/modules/core/role"
	"eventkan-backend/internal/modules/core/tenant"
	"eventkan-backend/internal/modules/core/user"
	"eventkan-backend/internal/modules/features/attendance"
	"eventkan-backend/internal/modules/features/certificate"
	"eventkan-backend/internal/modules/features/content"
	emailModule "eventkan-backend/internal/modules/features/email"
	"eventkan-backend/internal/modules/features/event"
	"eventkan-backend/internal/modules/features/payment"
	"eventkan-backend/internal/modules/features/registration"
	"eventkan-backend/internal/modules/features/support"
	"eventkan-backend/internal/modules/features/testimonial"

	"eventkan-backend/internal/shared/audit"
	"eventkan-backend/internal/shared/authz"
	sharedRedis "eventkan-backend/internal/shared/redis"
...
func Setup(router *gin.Engine, db *pgxpool.Pool, cfg *config.Config) {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:5173", "http://localhost:3000", "http://localhost:3001", "http://localhost:8002", "http://localhost:8081", "https://app.eventkan.id"},
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"status":  "ok",
			"message": "EVENTKAN API is running",
		})
	})
	...
	featuresV1 := router.Group("/features/v1")
	{
		...
		registrationModule := registration.Initialize(db)
		...
		emailModule.Initialize(db, cfg).SetupRoutes(featuresV1)
	}
