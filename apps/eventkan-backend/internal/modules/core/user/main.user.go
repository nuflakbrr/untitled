...
import (
	...
	"eventkan-backend/internal/modules/core/user/handler"
	"eventkan-backend/internal/modules/core/user/repository"
	"eventkan-backend/internal/modules/core/user/service"
)
...
func (m *UserModule) SetupRoutes(router *gin.RouterGroup) {
	...
	users.Use(middleware.JWTAuth())
	{
		...
		users.DELETE("/:id/permanent", middleware.RequirePermission("user.delete"), m.Handler.PermanentDelete)
