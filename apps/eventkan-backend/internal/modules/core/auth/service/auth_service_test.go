...
import (
	...
	authDto "eventkan-backend/internal/modules/core/auth/dto"
	tenantDomain "eventkan-backend/internal/modules/core/tenant/domain"
	tenantDto "eventkan-backend/internal/modules/core/tenant/dto"
	userDomain "eventkan-backend/internal/modules/core/user/domain"
...
func TestSignIn_Success(t *testing.T) {
	...
	resp, err := svc.SignIn(context.Background(), authDto.SignInRequest{
		Email:    "superadmin.univ@eventkan.ac.id",
		Password: "password",
	})
...
func TestSignIn_TemporaryBan_Expired_AllowsLogin(t *testing.T) {
	...
	_, err := svc.SignIn(context.Background(), authDto.SignInRequest{
		Email:    "expiredban@eventkan.ac.id",
		Password: "password",
	})
...
func TestSignUp_Success(t *testing.T) {
	...
	resp, err := svc.SignUp(context.Background(), authDto.SignUpRequest{
		Email:    "  Newpeserta@EVENTKAN.AC.ID  ",
		Name:     "  Peserta Baru  ",
	...
	if resp.User.Email != "newpeserta@eventkan.ac.id" {
	...
	uRepo := &mockUserRepo{
		findByIDFn: func(_ context.Context, _ string) (*userDomain.User, error) {
			return &userDomain.User{
				...
				Email:    "superadmin.fasilkom@eventkan.ac.id",
				Role:     "superadmin",
				TenantID: &myTenantID,
			}, nil
		},
