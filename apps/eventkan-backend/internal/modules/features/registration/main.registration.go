...
import (
	...
	"eventkan-backend/internal/modules/features/registration/handler"
	"eventkan-backend/internal/modules/features/registration/repository"
	"eventkan-backend/internal/modules/features/registration/service"
...
func (m *Module) SetupRoutes(router *gin.RouterGroup) {
	registrations := router.Group("/registrations", middleware.JWTAuth())
	{
		...
		registrations.GET("", middleware.RequirePermission("registrations.read"), m.Handler.ListAll)
