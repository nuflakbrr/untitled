...
import (
	...
	"eventkan-backend/internal/modules/features/registration/domain"
	"eventkan-backend/internal/modules/features/registration/dto"
	"eventkan-backend/internal/modules/features/registration/repository"
...
type RegistrationAllRepository interface {
	ListAll(ctx context.Context, scopeTenantID *string, query dto.RegistrationQuery) ([]*domain.Registration, int64, error)
}
...
func (s *RegistrationService) ListAll(ctx context.Context, scopeTenantID *string, query dto.RegistrationQuery) ([]dto.RegistrationResponse, int64, error) {
	repository, ok := s.repository.(RegistrationAllRepository)
	if !ok {
		return nil, 0, fmt.Errorf("list all registrations is unavailable")
	}
	registrations, total, err := repository.ListAll(ctx, scopeTenantID, query)
	if err != nil {
		return nil, 0, err
	}
	return toResponses(registrations), total, nil
...
func toResponse(registration *domain.Registration) dto.RegistrationResponse {
	return dto.RegistrationResponse{
		...
		EventStartDate: registration.EventStartDate, EventStartTime: registration.EventStartTime,
		EventEndTime: registration.EventEndTime, EventLocation: registration.EventLocation,
