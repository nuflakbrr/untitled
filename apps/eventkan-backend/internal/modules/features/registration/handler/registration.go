...
type AllRegistrationService interface {
	ListAll(ctx context.Context, scopeTenantID *string, query dto.RegistrationQuery) ([]dto.RegistrationResponse, int64, error)
}
...
func (h *RegistrationHandler) ListAll(c *gin.Context) {
	service, ok := h.service.(AllRegistrationService)
	if !ok {
		response.Error(c, http.StatusInternalServerError, "Registration listing is unavailable", "")
		return
	}
	var query dto.RegistrationQuery
	if err := c.ShouldBindQuery(&query); err != nil {
	...
	registrations, total, err := service.ListAll(c.Request.Context(), scope, query)
	if err != nil {
		writeRegistrationError(c, err, "Failed to retrieve registrations")
		return
	}
