...
import (
	...
	"eventkan-backend/internal/modules/features/event/service"
...
func (h *EventHandler) Restore(c *gin.Context) {
	_, scope, _, ok := actorScope(c)
	if !ok {
		return
	}
	service, ok := h.service.(interface {
		RestoreEvent(context.Context, string, *string) error
	})
	if !ok {
		response.Error(c, http.StatusNotImplemented, "Event restore unavailable", "")
		return
	}
	if err := service.RestoreEvent(c.Request.Context(), c.Param("id"), scope); err != nil {
		writeEventError(c, err, "Failed to restore event")
		return
	}
	response.Success(c, http.StatusOK, "Event restored successfully", nil)
}
