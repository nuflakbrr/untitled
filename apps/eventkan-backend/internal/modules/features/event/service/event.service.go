...
import (
	...
	"eventkan-backend/internal/modules/features/event/repository"
...
func (s *EventService) RestoreEvent(ctx context.Context, id string, scopeTenantID *string) error {
	restorer, ok := s.events.(interface {
		Restore(context.Context, string, *string) error
	})
	if !ok {
		return errors.New("event restore unavailable")
	}
	return restorer.Restore(ctx, id, scopeTenantID)
}

func (s *EventService) RestoreCategory(ctx context.Context, id string, scopeTenantID *string) error {
	restorer, ok := s.categories.(interface {
		Restore(context.Context, string, *string) error
	})
	if !ok {
		return errors.New("event category restore unavailable")
	}
	return restorer.Restore(ctx, id, scopeTenantID)
}
...
func toCategoryResponse(category *domain.Category) dto.CategoryResponse {
	return dto.CategoryResponse{ID: category.ID, TenantID: category.TenantID, Name: category.Name, Slug: category.Slug, Description: category.Description, DeletedAt: category.DeletedAt, EventsCount: category.EventsCount}
}
