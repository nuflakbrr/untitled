...
import (
	...
	"eventkan-backend/internal/modules/features/event/handler"
...
func (m *Module) SetupRoutes(router *gin.RouterGroup) {
	categories := router.Group("/event-categories")
	{
		...
		categories.PATCH("/:id/restore", middleware.JWTAuth(), middleware.RequirePermission("event.categories.delete"), m.CategoryHandler.Restore)
	}
	...
	events := router.Group("/events")
	{
		...
		events.PATCH("/:id/restore", middleware.JWTAuth(), middleware.RequirePermission("events.delete"), m.EventHandler.Restore)
	}
}
