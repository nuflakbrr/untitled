...
import (
	...
	"eventkan-backend/internal/modules/features/content/handler"
	"eventkan-backend/internal/modules/features/content/repository"
	"eventkan-backend/internal/modules/features/content/service"
...
func (m *Module) SetupRoutes(router *gin.RouterGroup) {
	articles := router.Group("/articles")
	{
		...
		articles.PATCH("/:id/restore", middleware.JWTAuth(), middleware.RequirePermission("articles.delete"), m.ArticleHandler.Restore)
		articles.DELETE("/:id/permanent", middleware.JWTAuth(), middleware.RequirePermission("articles.delete"), m.ArticleHandler.PermanentDelete)
	}
	...
	categories := router.Group("/article-categories")
	{
		...
		categories.PATCH("/:id/restore", middleware.JWTAuth(), middleware.RequirePermission("article.categories.delete"), m.CategoryHandler.Restore)
		categories.DELETE("/:id/permanent", middleware.JWTAuth(), middleware.RequirePermission("article.categories.delete"), m.CategoryHandler.PermanentDelete)
	}

	galleries := router.Group("/galleries")
	{
		galleries.GET("", m.GalleryHandler.List)
		galleries.GET("/:id", m.GalleryHandler.GetByID)
		...
		galleries.PATCH("/:id/restore", middleware.JWTAuth(), middleware.RequirePermission("galleries.delete"), m.GalleryHandler.Restore)
		galleries.DELETE("/:id/permanent", middleware.JWTAuth(), middleware.RequirePermission("galleries.delete"), m.GalleryHandler.PermanentDelete)
	}
}
