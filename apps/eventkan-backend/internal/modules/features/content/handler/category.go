...
type categoryLifecycle interface {
	RestoreCategory(context.Context, string) error
	PermanentDeleteCategory(context.Context, string) error
}
type categoryListDeleted interface {
	ListCategoriesWithDeleted(context.Context) ([]dto.CategoryResponse, error)
}
...
func (h *CategoryHandler) List(c *gin.Context) {
	...
	if c.Query("include_deleted") == "true" {
		service, ok := h.service.(categoryListDeleted)
		if !ok {
			response.Error(c, 500, "Lifecycle unavailable", "")
			return
		}
		categories, err = service.ListCategoriesWithDeleted(c.Request.Context())
	} else {
...
func (h *CategoryHandler) Restore(c *gin.Context) {
	s, ok := h.service.(categoryLifecycle)
	if !ok {
		response.Error(c, 500, "Lifecycle unavailable", "")
		return
	}
	if err := s.RestoreCategory(c, c.Param("id")); err != nil {
		writeCategoryError(c, err, "Failed to restore category")
		return
	}
	response.Success(c, 200, "Category restored successfully", nil)
}
func (h *CategoryHandler) PermanentDelete(c *gin.Context) {
	...
	if err := s.PermanentDeleteCategory(c, c.Param("id")); err != nil {
		writeCategoryError(c, err, "Failed to permanently delete category")
		return
	}
	response.Success(c, 200, "Category permanently deleted", nil)
}
