...
type articleLifecycle interface {
	RestoreArticle(context.Context, string, *string) error
	PermanentDeleteArticle(context.Context, string, *string) error
}
...
func (h *ArticleHandler) Restore(c *gin.Context) {
	...
	s, ok := h.service.(articleLifecycle)
	if !ok {
		response.Error(c, 500, "Lifecycle unavailable", "")
		return
	}
	if err := s.RestoreArticle(c, c.Param("id"), scope); err != nil {
		writeArticleError(c, err, "Failed to restore article")
		return
	}
	response.Success(c, 200, "Article restored successfully", nil)
}
func (h *ArticleHandler) PermanentDelete(c *gin.Context) {
	...
	if err := s.PermanentDeleteArticle(c, c.Param("id"), scope); err != nil {
		writeArticleError(c, err, "Failed to permanently delete article")
		return
	}
	response.Success(c, 200, "Article permanently deleted", nil)
}
