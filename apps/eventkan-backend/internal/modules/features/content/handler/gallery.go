...
type galleryLifecycle interface {
	RestoreGallery(context.Context, string, *string) error
	PermanentDeleteGallery(context.Context, string, *string) error
}

type GalleryDetailService interface {
...
func (h *GalleryHandler) GetByID(c *gin.Context) {
	service, ok := h.service.(GalleryDetailService)
	if !ok {
		response.Error(c, http.StatusInternalServerError, "Gallery detail is unavailable", "")
		return
	}
	gallery, err := service.GetGalleryByID(c.Request.Context(), c.Param("id"))
	if err != nil {
		writeGalleryError(c, err, "Failed to retrieve gallery")
		return
	}
	response.Success(c, http.StatusOK, "Gallery retrieved successfully", gallery)
}
...
func (h *GalleryHandler) Restore(c *gin.Context) {
	scope, ok := organizerScope(c)
	if !ok {
		return
	}
	s, ok := h.service.(galleryLifecycle)
	if !ok {
		response.Error(c, 500, "Lifecycle unavailable", "")
		return
	}
	if err := s.RestoreGallery(c, c.Param("id"), scope); err != nil {
		writeGalleryError(c, err, "Failed to restore gallery")
		return
	}
	response.Success(c, 200, "Gallery restored successfully", nil)
}
func (h *GalleryHandler) PermanentDelete(c *gin.Context) {
	scope, ok := organizerScope(c)
	if !ok {
		return
	}
	...
	if err := s.PermanentDeleteGallery(c, c.Param("id"), scope); err != nil {
		writeGalleryError(c, err, "Failed to permanently delete gallery")
		return
	}
	response.Success(c, 200, "Gallery permanently deleted", nil)
}
