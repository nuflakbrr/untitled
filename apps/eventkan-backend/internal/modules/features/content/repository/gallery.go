...
func (r *GalleryRepository) FindByID(ctx context.Context, id string) (*domain.Gallery, error) {
	...
	err := r.db.QueryRow(ctx, gallerySelect+" WHERE id = $1 AND deleted_at IS NULL", id).Scan(
...
func (r *GalleryRepository) Delete(ctx context.Context, id string, scopeTenantID *string) error {
	query := "UPDATE galleries SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL"
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
		query += fmt.Sprintf(" AND tenant_id = $%d", len(args))
...
func (r *GalleryRepository) Restore(ctx context.Context, id string, scopeTenantID *string) error {
	q := "UPDATE galleries SET deleted_at = NULL, updated_at = NOW() WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
		q += " AND tenant_id = $2"
...
func (r *GalleryRepository) PermanentDelete(ctx context.Context, id string, scopeTenantID *string) error {
	q := "DELETE FROM galleries WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
		q += " AND tenant_id = $2"
