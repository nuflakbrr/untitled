...
const articleSelect = `
	SELECT a.id, a.tenant_id, a.title, a.content, a.cover, a.slug, a.created_by_id, COALESCE(u.name, ''), a.created_at, a.updated_at
	FROM articles a LEFT JOIN core.users u ON u.id = a.created_by_id`
...
func (r *ArticleRepository) Delete(ctx context.Context, id string, scopeTenantID *string) error {
	query := "UPDATE articles SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL"
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
		query += fmt.Sprintf(" AND tenant_id = $%d", len(args))
...
func (r *ArticleRepository) Restore(ctx context.Context, id string, scopeTenantID *string) error {
	return r.updateDeletedAt(ctx, id, scopeTenantID, false)
}
func (r *ArticleRepository) PermanentDelete(ctx context.Context, id string, scopeTenantID *string) error {
	q := "DELETE FROM articles WHERE id = $1 AND deleted_at IS NOT NULL"
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
		q += " AND tenant_id = $2"
...
func (r *ArticleRepository) updateDeletedAt(ctx context.Context, id string, scopeTenantID *string, restore bool) error {
	value := "NOW()"
	if restore {
		value = "NULL"
	}
	q := fmt.Sprintf("UPDATE articles SET deleted_at = %s, updated_at = NOW() WHERE id = $1 AND deleted_at IS %s", value, map[bool]string{true: "NOT NULL", false: "NULL"}[restore])
	args := []any{id}
	if scopeTenantID != nil {
		args = append(args, *scopeTenantID)
...
func (r *ArticleRepository) scanOne(ctx context.Context, query string, args ...any) (*domain.Article, error) {
	...
	err := r.db.QueryRow(ctx, query, args...).Scan(
		...
		&article.Cover, &article.Slug, &article.CreatedByID, &article.CreatedByName, &article.CreatedAt, &article.UpdatedAt,
	)
