...
func (r *CategoryRepository) FindAll(ctx context.Context) ([]*domain.ArticleCategory, error) {
	rows, err := r.db.Query(ctx, `SELECT id, name, created_at, updated_at FROM article_categories WHERE deleted_at IS NULL ORDER BY name ASC`)
	if err != nil {
...
func (r *CategoryRepository) FindAllWithDeleted(ctx context.Context) ([]*domain.ArticleCategory, error) {
	rows, err := r.db.Query(ctx, `SELECT id, name, created_at, updated_at, deleted_at FROM article_categories ORDER BY name ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := make([]*domain.ArticleCategory, 0)
	for rows.Next() {
		item := new(domain.ArticleCategory)
...
func (r *CategoryRepository) Update(ctx context.Context, category *domain.ArticleCategory) error {
	err := r.db.QueryRow(ctx, `
		UPDATE article_categories SET name = $2, updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL
...
func (r *CategoryRepository) Delete(ctx context.Context, id string) error {
	tag, err := r.db.Exec(ctx, `UPDATE article_categories SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL`, id)
	if err != nil {
...
func (r *CategoryRepository) Restore(ctx context.Context, id string) error {
	_, err := r.db.Exec(ctx, `UPDATE article_categories SET deleted_at = NULL, updated_at = NOW() WHERE id=$1 AND deleted_at IS NOT NULL`, id)
	return err
}
func (r *CategoryRepository) PermanentDelete(ctx context.Context, id string) error {
	_, err := r.db.Exec(ctx, `DELETE FROM article_categories WHERE id=$1 AND deleted_at IS NOT NULL`, id)
	return err
}
