...
> **Database**: PostgreSQL (UUID PKs, `golang-migrate`, SQL Seeders)  
> **Authentication**: JWT & PBAC (Permission-Based Access Control) with Redis Cache + Better Auth compatible schema  
> **Last Updated**: 2026
...
1. [01_PROJECT.md](./PRD/01_PROJECT.md) - Tentang SITIVENT, deskripsi sistem, target jenis event, status fitur saat ini, dan roadmap masa depan.
...
3. [03_ARCHITECTURE.md](./PRD/03_ARCHITECTURE.md) - Prinsip arsitektur monorepo, REST API layer (`apps/backend`), Next.js App Router (`apps/frontend`), dan komunikasi antarlayanan.
...
7. [07_DATABASE.md](./PRD/07_DATABASE.md) - Konvensi skema PostgreSQL, UUID PKs, `golang-migrate` (1 migration = 1 table), SQL seeders idempotent.
...
9. [09_FEATURES.md](./PRD/09_FEATURES.md) - Ringkasan seluruh modul dan fitur utama sistem SITIVENT.
10. [10_EVENT_MODULE.md](./PRD/10_EVENT_MODULE.md) - Alur kerja event, kategori, pembicara, fasilitas, online/offline, status lifecycle, dan validasi kuota.
11. [11_REGISTRATION_MODULE.md](./PRD/11_REGISTRATION_MODULE.md) - Alur pendaftaran peserta, nomor registrasi, QR token, dan validasi ketersediaan.
12. [12_PAYMENT_MODULE.md](./PRD/12_PAYMENT_MODULE.md) - Alur pembayaran manual, verifikasi bukti transfer admin, status pembayaran, refund, dan audit trail.
...
14. [14_CERTIFICATE_MODULE.md](./PRD/14_CERTIFICATE_MODULE.md) - Template builder dinamis, custom styling/fonts, multi-signature, generate sertifikat otomatis untuk peserta hadir, dan halaman verifikasi publik.
15. [15_EMAILS.md](./PRD/15_EMAILS.md) - Arsitektur antrean email asinkron (`email_queues`), jenis email transaksional, dan integrasi SMTP Go / Nodemailer.
...
17. [17_AUTHORIZATION.md](./PRD/17_AUTHORIZATION.md) - Autentikasi JWT / Better Auth, model PBAC (User, Role, Permission) dengan cache Redis di Go middleware.
18. [18_SECURITY.md](./PRD/18_SECURITY.md) - Kebijakan keamanan, validasi input Zod/Go validator, sanitasi data, CSRF/CORS, security headers, dan audit logging.
...
22. [22_TESTING.md](./PRD/22_TESTING.md) - Pengujian backend Go (`go test`), frontend E2E dengan Playwright, dan reset database seed.
